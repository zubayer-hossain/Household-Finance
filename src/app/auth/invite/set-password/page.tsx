...
import { InviteSetPasswordForm } from "@/features/auth/components/InviteSetPasswordForm";
import { userNeedsPasswordSet } from "@/features/auth/lib/user-password-setup";
...
export default function InviteSetPasswordPage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.replace(
        `/login?reason=invite&message=${encodeURIComponent(
  ...
  if (!user || !userNeedsPasswordSet(user)) {
    return (
      <div className="surface-market flex min-h-dvh flex-col items-center justify-center gap-4 px-6 py-14">
